 ...
 * implementation
 * encapsulates the steps needed to take preview-sized images, which are used for both preview
 * and decoding.
 *
...
public final class CameraManager {
    ...
    /**
     * Preview frames are delivered here, which we pass on to the registered handler. Make sure
     * to clear the handler so
     ...
     * byte[] in the
     * message.obj field, with width and height encoded as message.arg1 and message.arg2,
     * respectively.
     *
     ...
     * barcode. This target
     * helps with alignment as well as forces the user to hold the device far enough away to
     * ensure the image will be in
     * focus.
     *
...
//            int topOffset = (screenResolution.y - height) / 2;
     ...
     * getFramingRectInPreview(); int count =
     * points.length; Point[] output = new Point[count]; for (int x = 0; x < count; x++) {
     * output[x] = new Point();
     * output[x].x = frame.left + (int) (points[x].getX() + 0.5f); output[x].y = frame.top +
     * (int) (points[x].getY() +
     * 0.5f); } return output; }
     */

    /**
     * A factory method to build the appropriate LuminanceSource object based on the format of
     * the preview buffers, as
